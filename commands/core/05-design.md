...
```
  ⚠ This project does not appear to have a graphical user
    interface. /titan:05-design is for projects with visual UI.
    ...
    → /titan:06-plan to create Phase 1 execution plan
```
...
> Generated by /titan:05-design on [date]
...
Update Next Action: "Run `/titan:06-plan` to create Phase 1 execution plan."
...
| User approves but wants changes later | That is fine. They can re-run `/titan:05-design` to iterate further, or make changes during the build phase. Note in STATE.md. |
...
- **After `/titan:04-explore`:** Exploration may have identified UI patterns or libraries to use.
- **Before `/titan:06-plan`:** Plans reference approved mockups as implementation targets.
- **During `/titan:07-build`:** Executor agents reference mockups and SPEC.md for pixel-accurate implementation.
- **During `/titan:08-verify`:** Verifier checks implementation against approved mockups and SPEC.md.
...
> Run `/titan:06-plan` to create the **Phase 1 execution plan**. Your approved designs will be used as implementation targets.

### Other options

| Command | Action |
|---------|--------|
| `/titan:04-explore` | Research technologies or unknowns before planning |
| `/titan:05-design` | Re-run to iterate on designs or add more screens |
