...
- `.titan/PROJECT.md` exists (run `/titan:vision` first)
- `.titan/REQUIREMENTS.md` exists (run `/titan:vision` first)
...
```
  ⚠ This project does not appear to have a graphical user
    interface. /titan:design is for projects with visual UI.
    ...
    → /titan:plan to create Phase 1 execution plan
```
...
> Generated by /titan:design on [date]
...
Update Next Action: "Run `/titan:plan` to create Phase 1 execution plan."
...
| User approves but wants changes later | That is fine. They can re-run `/titan:design` to iterate further, or make changes during the build phase. Note in STATE.md. |
...
- **After `/titan:vision`:** Design uses PROJECT.md for user context and REQUIREMENTS.md for feature scope.
- **After `/titan:explore`:** Exploration may have identified UI patterns or libraries to use.
- **Before `/titan:plan`:** Plans reference approved mockups as implementation targets.
